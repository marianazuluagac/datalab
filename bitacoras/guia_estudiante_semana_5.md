...
|---|---|---|---|---|---|---|---|
| 1 | `INSERT` válido `cientifico_datos` + `proyecto` | exitoso | exitoso | INSERT INTO experimento (id_proyecto, id_cientifico, configuracion) VALUES (999, 1, 'lr=0.01;epochs=50;batch=32')
 | — | | |
| 2 | `INSERT` `experimento` con `id_proyecto=999` | error | error | INSERT INTO experimento (id_proyecto, id_cientifico, configuracion) VALUES (999, 1, 'lr=0.01;epochs=50;batch=32')
 | `fk_experimento_proyecto` | | |
| 3 | `INSERT` válido `experimento` + `modelo` | exitoso | exitoso | INSERT INTO modelo (id_experimento, nombre, version, algoritmo) VALUES (2, 'modelo_fraude_v1', '1.0', 'Random Forest')
 | — | | |
| 4 | `INSERT` `metrica` con `valor=1.5` | error (el valor estaba fuera dle rango) | exitoso | INSERT INTO metrica (id_modelo, nombre_metrica, valor, fecha_calculo) VALUES (3, 'accuracy', 0.1, CURDATE())
 | `chk_metrica_valor` | | |
| 5 | `DELETE` `proyecto` con hijos | ERROR (EN PROYECTO NO HAY NADA CREADO, POR LO CUAL NO VA A ELMINAR NADA) | ERROR | DELETE FROM proyecto WHERE id_proyecto = 1
| `fk_experimento_proyecto` | | |
| 6 | `DELETE` `modelo` con métrica | EXITOSO | EXITOSO | INSERT INTO metrica (id_modelo, nombre_metrica, valor, fecha_calculo) VALUES (3, 'accuracy', 0.1, CURDATE())  ////   DELETE FROM modelo WHERE id_modelo = 1
 | `fk_metrica_modelo` | | |
